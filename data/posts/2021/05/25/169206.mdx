...
これでUI Builder Editor Windowが開きます.　デフォルトで&lt;unsaved file&gt; *.uxmlが生成されるのでペインを選択してCtrl + Sで適当な名前(test.uxmlなど)保存する.
...
を選択する. 名前を付けて保存し, クリックするとUI Builder Editor Windowが表示される.
